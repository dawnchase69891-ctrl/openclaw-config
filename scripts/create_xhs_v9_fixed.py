...
WIDTH = 1242
HEIGHT = 1660
TARGET_RATIO = WIDTH / HEIGHT  # 0.748
...
def smart_crop_image(img, target_width, target_height):
    """
    智能裁剪图片，保持比例不变形
    只裁剪，绝不拉伸
    """
    ...
    if img_ratio > target_ratio:
        ...
        top = int((img.height - new_height) * 0.3)  # 偏上裁剪，保留主体
        if top < 0:
            top = 0
        if top + new_height > img.height:
...
def create_card_1(image_path, output_path):
    ...
    if os.path.exists(image_path):
        ...
        bg_height = int(HEIGHT * 0.60)
        bg_img = smart_crop_image(img, WIDTH, bg_height)
    ...
    symptoms = ["眼睛干涩刺痛", "看屏幕字重影", "眼袋黑眼圈明显", "充满红血丝"]
    for s in symptoms:
    ...
    draw.text((40, HEIGHT-45), "赤兔养生计划", font=load_font(28), fill=hex_to_rgb(COLORS['text_light']))
...
def create_card_2(image_path, output_path):
    ...
    y += 50
    for s in ["枸杞玉米黄质吸收蓝光", "菊花木犀草素抗氧化", "两者搭配效果提升3倍"]:
...
def create_card_3(image_path, output_path):
    ...
    y += 50
    for p in ["枸杞补 - 滋养肝血", "菊花清 - 清除肝火", "决明子通 - 润肠通便"]:
    ...
    y += 50
    for t in ["枸杞：宁夏中宁，暗红饱满", "菊花：胎菊最好，花朵完整", "决明子：颗粒饱满无杂质"]:
...
def create_card_4(image_path, output_path):
    ...
    if os.path.exists(image_path):
        ...
        left_img = smart_crop_image(img, left_width, HEIGHT)
    ...
    steps = [("1", "材料清洗", "枸杞快速冲洗"), ("2", "放入杯中", "决明子先放"), ("3", "热水冲泡", "85度焖泡5分钟")]
    ...
    y += 40
    for t in ["可加蜂蜜调味", "可续水2-3次", "枸杞直接吃掉"]:
...
def create_card_5(image_path, output_path):
    ...
    if os.path.exists(image_path):
        img = Image.open(image_path)
        bg_img = smart_crop_image(img, WIDTH, HEIGHT)
        canvas.paste(bg_img, (0, 0))
    ...
    effects = [("清肝明目", "1周后眼睛不再干涩"), ("滋补肝肾", "2周后视力更清晰"), ("抗氧抗衰", "1月后眼袋淡化")]
    ...
    y += 50
    for t in ["1周 -> 眼干缓解", "2周 -> 视力清晰", "1月 -> 黑眼圈淡化"]:
...
def create_card_6(output_path):
    ...
    y += 45
    for t in ["最佳时间：下午3-5点", "最佳频率：每天1杯", "最佳温度：温热饮用"]:
    ...
    y += 45
    for f in ["孕妇慎用决明子", "脾胃虚寒者饭后饮用", "感冒发烧时暂停"]:
    ...
    y += 45
    for q in ["Q: 喝了拉肚子？A: 减量或饭后喝", "Q: 可以不加决明子？A: 可以但效果打折扣"]:
    ...
    y += 45
    for i in ["点赞收藏，下次翻出来", "评论区晒你的变化", "关注解锁更多养生秘籍"]:
...
def main():
    output_dir = os.path.expanduser("~/.openclaw/media/xhs-v9/")
